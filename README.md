...
备注：本项目以提供计算工具以及运控管理框架为主，不提供与卫星实际操作控制相关的任何指令。
...
1. 其中星座任务规划服务与指令传输或遥测采集服务将会根据具体卫星星座的具体功能而定。这两个模块将是项目最具差异性的关键性模块。
2. 针对不同的星座或卫星有着不同的任务指令。任务规划服务能够规划多少种任务，取决于拥有的任务类型编码数量。在任务规划服务中不保存任何指令信息。
3. 根据任务类型编码进行指令的封装，这个操作由指令传输或遥测采集服务模块负责完成。
4. 星座任务调度服务是一个分布式无状态的集群服务。

## 星座管理模式

### 正常模式

当卫星轨道或其他影响通信的要素确定后，星座就进入正常模式。可以对卫星发起任务规划。

### 更新模式

当星座中的卫星轨道发生变化时，将会重新计算星间网络以及星地网络通信日历，并重新对任务进行排期，在该过程中，将会暂停任务的创建与调度下发。为此将会暂停用户新建任务规划。

备注：当星座中的卫星数据较多时，为了避免卫星轨道的更新导致星座频繁进入更新模式，影响对星座的管理效率，支持一次更新多星轨道。
...
| 版本 | 发送方 | 接收方                 | 内容                 |
| ---- | ------ | ---------------------- | -------------------- |
| 0.3  | 未知   | 卫星瞬根更新服务       | 接收卫星轨道瞬根服务 |
| 0.3  | 未知   | 指令传输或遥测采集服务 | 接收运控指令执行回执 |
...
| 0.3  | 获取卫星的位置信息信息     | 卫星位置信息         | 星座任务规划服务 |
| 0.3  | 获取信关站位置信息         | 卫星数据配置服务     | 星座任务规划服务 |
| 0.3  | 获取星站组网时间窗         | 星站组网时间窗计算   | 星座任务规划服务 |
| 0.3  | 获取星地通信日历           | 星地通信时间日历计算 | 星座任务规划服务 |
| 0.3  | 添加任务到执行调度         | 星座任务调度服务     | 星座任务规划服务 |
| 0.3  | 暂停执行特定任务           | 星座任务调度服务     | 星座任务规划服务 |
| 0.3  | 取消执行特定任务           | 星座任务调度服务     | 星座任务规划服务 |
| 0.3  | 根据新日历对任务重新排期   | 星座任务调度服务     | 星座任务规划服务 |
| 0.3  | 调整指定任务的执行排期     | 星座任务调度服务     | 星座任务规划服务 |
| 0.3  | 发布系统进入更新模式       | 微服务配置中心服务   | 卫星瞬根更新服务 |
| 0.3  | 发布系统进行正常模式       | 微服务配置中心服务   | 星座任务规划服务 |
...
| 字段名           | 類型         | 含義                                        | 説明   |
| ---------------- | ------------ | ------------------------------------------- | ------ |
| NoradId          | Char(5)      | 北美防空司令部給衛星分配的編號              | 必須   |
| OpenId           | Char(32)     | 本系統給衛星指定的一個                      | 可為空 |
